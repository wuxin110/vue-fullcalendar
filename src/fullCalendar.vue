    ...
    <div class="full-calendar-body">
      ...
      <div class="dates" ref="dates">
        ...
        <div class="dates-events">
          <div class="events-week" :style="'margin-top: '+(9)+'px'" v-for="(week,index) in currentDates">
            <div class="events-day" v-for="day in week" track-by="$index"
                 :class="{'today' : day.isToday,
              'not-cur-month' : !day.isCurMonth}" @click.stop="dayClick(day.date, $event)">
              <p v-if="tpl==0" class="day-number" style="width: 50px;float: right;padding: 0">{{ day.monthDay }}</p>
              <p v-if="tpl!=0" class="day-number">{{ day.monthDay }}</p>
              <div v-if="tpl==0" class="event-box">
                <event-card :tpl="tpl" :event="event" :date="day.date" :firstDay="firstDay" v-for="event in day.events" v-show="event.cellIndex <= eventLimit"  @click="eventClick">
                  <template scope="p">
  ...
  export default {
    ...
    methods : {
      emitChangeMonth (firstDayOfMonth) {
        ...
        this.$emit('changeMonth', start, end, firstDayOfMonth,(myDate,foramatString)=> {
            return myDate.locale(this.locale).format(foramatString);
        })
      },
      moreTitle (date) {
        if (!date) return '';
